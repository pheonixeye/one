...
class PiLabsApi extends PiApi<PiLab> {
  final String doc_id;

  PiLabsApi({required this.doc_id});

  static const String collection = 'labs';
  ...
  @override
  Future<ApiResult<List<PiLab>>> fetchDoctorItems() async {
    try {
      ...
      final _items = _result.items
          .map((e) => PiLab.fromJson(e.toJson()))
          .toList();

      return ApiDataResult<List<PiLab>>(data: _items);
    } catch (e) {
      return ApiErrorResult<List<PiLab>>(
