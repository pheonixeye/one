...
class PiProceduresApi extends PiApi<PiProcedure> {
  final String doc_id;

  PiProceduresApi({required this.doc_id});

  static const String collection = 'procedures';
  ...
  @override
  Future<ApiResult<List<PiProcedure>>> fetchDoctorItems() async {
    try {
      ...
      final _items = _result.items
          .map((e) => PiProcedure.fromJson(e.toJson()))
          .toList();

      return ApiDataResult<List<PiProcedure>>(data: _items);
    } catch (e) {
      return ApiErrorResult<List<PiProcedure>>(
