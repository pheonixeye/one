...
class PiRadsApi extends PiApi<PiRad> {
  final String doc_id;

  PiRadsApi({required this.doc_id});

  static const String collection = 'rads';
  ...
  @override
  Future<ApiResult<List<PiRad>>> fetchDoctorItems() async {
    try {
      ...
      final _items = _result.items
          .map((e) => PiRad.fromJson(e.toJson()))
          .toList();

      return ApiDataResult<List<PiRad>>(data: _items);
    } catch (e) {
      return ApiErrorResult<List<PiRad>>(
