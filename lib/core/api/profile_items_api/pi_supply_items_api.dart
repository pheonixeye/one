...
class PiSupplyItemsApi extends PiApi<PiSupplyItem> {
  final String doc_id;

  PiSupplyItemsApi({required this.doc_id});

  static const String collection = 'supplies';
  ...
  @override
  Future<ApiResult<List<PiSupplyItem>>> fetchDoctorItems() async {
    try {
      ...
      final _items = _result.items
          .map((e) => PiSupplyItem.fromJson(e.toJson()))
          .toList();

      return ApiDataResult<List<PiSupplyItem>>(data: _items);
    } catch (e) {
      return ApiErrorResult<List<PiSupplyItem>>(
