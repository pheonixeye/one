...
class _PiDocumentTypeCreateEditDialogState
  ...
  void initState() {
    ...
    _nameEnController = TextEditingController(
      text: widget.piDocumentType?.name_en ?? '',
    );
    _nameArController = TextEditingController(
      text: widget.piDocumentType?.name_ar ?? '',
    );

    ///[DOCUMENTS]
    ///
    _is_allowed_on_portal =
        widget.piDocumentType?.is_allowed_on_portal ?? false;
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Row(
        children: [
          Expanded(
            child: widget.piDocumentType == null
                ? Text.rich(
                    TextSpan(
                      ...
                      children: [
                        ...
                        TextSpan(
                          text:
                              '(${ProfileSetupItem.documents.pageTitleName(context)})',
                          style: TextStyle(
                            fontSize: 12,
        ...
        ElevatedButton.icon(
          onPressed: () {
            if (formKey.currentState!.validate()) {
              ...
              final _piDocumentType = PiDocumentType.fromJson(_itemJson);

              Navigator.pop(context, _piDocumentType);
            }
          },
