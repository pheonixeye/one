...
import 'package:one/pages/loading_page/pages/lang_page/pages/shell_page/pages/app_page/pages/app_profile_setup/pages/pi_labs_page/pi_lab_view_card.dart';
...
class PiLabsPage extends StatefulWidget {
  const PiLabsPage({super.key});

  @override
  State<PiLabsPage> createState() => _PiLabsPageState();
}

class _PiLabsPageState extends State<PiLabsPage> {
      ...
      builder: (context, pl, l, _) {
        return Scaffold(
                  ...
                  child: ListTile(
                    title: Row(
                      children: [
                        ...
                        Text(ProfileSetupItem.labs.pageTitleName(context)),
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      ...
                      while (pl.labs is ApiErrorResult) {
                        return CentralError(
                          code: (pl.labs as ApiErrorResult).errorCode,
                          toExecute: pl.retry,
                        );
                      }

                      while (pl.labs != null &&
                          (pl.labs is ApiDataResult) &&
                          (pl.labs as ApiDataResult<List<PiLab>>)
                              .data
                              .isEmpty) {
                        return CentralNoItems(
                          message:
                              '${context.loc.noItemsFound}\n(${PiLab.item.pageTitleName(context)})',
                        );
                      }
                      final _items =
                          (pl.filteredLabs as ApiDataResult<List<PiLab>>).data;
                      return ListView.builder(
                        itemCount: _items.length,
                        cacheExtent: 3000,
                        itemBuilder: (context, index) {
                          final _piLab = _items[index];
                          return PiLabViewCard(
                            piLab: _piLab,
                            index: index,
                          );
                        },
          ...
          floatingActionButton: SmBtn(
            tooltip: ProfileSetupItem.labs.actionButtonTooltip(context),
            onPressed: () async {
              //@permission
