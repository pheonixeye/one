...
import 'package:one/pages/loading_page/pages/lang_page/pages/shell_page/pages/app_page/pages/app_profile_setup/pages/pi_rads_page/pi_rad_view_card.dart';
...
class PiRadsPage extends StatefulWidget {
  const PiRadsPage({super.key});

  @override
  State<PiRadsPage> createState() => _PiRadsPageState();
}

class _PiRadsPageState extends State<PiRadsPage> {
      ...
      builder: (context, r, l, _) {
        return Scaffold(
                  ...
                  child: ListTile(
                    title: Row(
                      children: [
                        ...
                        Text(ProfileSetupItem.rads.pageTitleName(context)),
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      ...
                      while (r.rads is ApiErrorResult) {
                        return CentralError(
                          code: (r.rads as ApiErrorResult).errorCode,
                          toExecute: r.retry,
                        );
                      }

                      while (r.rads != null &&
                          (r.rads is ApiDataResult) &&
                          (r.rads as ApiDataResult<List<PiRad>>).data.isEmpty) {
                        return CentralNoItems(
                          message:
                              '${context.loc.noItemsFound}\n(${PiRad.item.pageTitleName(context)})',
                        );
                      }
                      final _items =
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final _piRad = _items[index];
                          return PiRadViewCard(
                            piRad: _piRad,
                            index: index,
                          );
                        },
          ...
          floatingActionButton: SmBtn(
            tooltip: ProfileSetupItem.rads.actionButtonTooltip(context),
            onPressed: () async {
              //@permission
