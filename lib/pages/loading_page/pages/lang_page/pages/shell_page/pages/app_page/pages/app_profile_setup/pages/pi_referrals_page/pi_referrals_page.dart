...
import 'package:one/pages/loading_page/pages/lang_page/pages/shell_page/pages/app_page/pages/app_profile_setup/pages/pi_referrals_page/pi_referral_view_card.dart';
...
class PiReferralsPage extends StatefulWidget {
  const PiReferralsPage({super.key});

  @override
  State<PiReferralsPage> createState() => _PiReferralsPageState();
}

class _PiReferralsPageState extends State<PiReferralsPage> {
                  ...
                  child: ListTile(
                    title: Row(
                      children: [
                        ...
                        Text(ProfileSetupItem.referrals.pageTitleName(context)),
                  ...
                  child: Builder(
                    builder: (context) {
                      ...
                      while (r.referrals is ApiErrorResult) {
                        return CentralError(
                          code: (r.referrals as ApiErrorResult).errorCode,
                          toExecute: r.retry,
                        );
                      }

                      while (r.referrals != null &&
                          (r.referrals is ApiDataResult) &&
                          (r.referrals as ApiDataResult<List<PiReferral>>)
                              .data
                              .isEmpty) {
                        return CentralNoItems(
                          message:
                              '${context.loc.noItemsFound}\n(${PiReferral.item.pageTitleName(context)})',
                        );
                      }
                      final _items =
                          (r.filteredReferrals
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final _piReferral = _items[index];
                          return PiReferralViewCard(
                            piReferral: _piReferral,
                            index: index,
                          );
                        },
                      );
            ...
            onPressed: () async {
              ...
              if (context.mounted) {
                await shellFunction(
                  context,
                  toExecute: () async {
                    await r.createItem(_piReferral);
                  },
                );
              }
            },
